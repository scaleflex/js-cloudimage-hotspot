...
### 1. `open()` on already-visible popover causes flicker
**File:** `src/core/ci-hotspot.ts`
`open(id)` calls `closeAll()` then re-shows. If the popover is already visible, this triggers spurious `onClose`/`onOpen` callbacks and a focus flicker. Fix: early-return if `popover.isVisible()`.

### 2. React hook: `scrollHint` missing from update payload
**File:** `src/react/use-ci-hotspot.ts:73-96`
`scrollHint` is included in the constructor config but not in the update effect's object, so dynamic changes are ignored. `cloudimage` is also missing from the deps array.

### 3. `SAFE_SRC_PATTERN` percent-encoding bypass (defense-in-depth)
**File:** `src/popover/sanitize.ts`
`data:image/svg%2Bxml` bypasses the negative lookahead since `%2B` != `+` at regex time. Not exploitable (`<img>` sandboxes SVG), but could be tightened.

---

## Code Quality (medium)

### 4. Popover hover listeners not cleaned via cleanup pattern
...
### 5. `updateHotspot()` could update marker in-place for simple changes
**File:** `src/core/ci-hotspot.ts`
...
### 6. `showLoadTriggerPopovers` and `setupResponsive` use `.find()` loops
...
### 7. React `JSON.stringify` on every render for deps comparison
...
### 8. `bindTrigger` method is ~100 lines long
**File:** `src/core/ci-hotspot.ts`
...
### 9. Zoom controls lack `aria-label` on buttons
...
### 10. No keyboard hint for scene navigation hotspots
...
### 11. Editor property panel inputs lack `id`/`for` pairing
...
### 12. No `destroyed` guard on editor public methods
...
### 13. Editor drag manager: no throttle on `pointermove`
...
### 14. Popover `max-width` clips long content without scroll
**File:** `src/styles/index.css`
Popover has `max-width: 320px` but no max-height or overflow handling. Long content overflows visually.

### 15. CSS: `--ci-hotspot-marker-border` uses shorthand in custom property
...
### 18. Pre-existing: `load trigger` test fails in jsdom
...
### 19. Pre-existing: React tests can't run (missing `@testing-library/dom`)
